...
__global__ void process(int width, int height, float* r, float* g, float* b)
{
    ...
    if (global_pixel_col < width &&  global_pixel_row < height)
    {
        ...
        int left_neighbor_index = (global_pixel_col - 1) + global_pixel_row * width;
        int right_neighbor_index = (global_pixel_col + 1) + global_pixel_row * width;
        int down_neighbor_index = (global_pixel_col) + (global_pixel_row + 1)* width;
        int up_neighbor_index = (global_pixel_col) + (global_pixel_row - 1) * width;

        float f_left = (r[left_neighbor_index] + g[left_neighbor_index] + b[left_neighbor_index]);
        float f_right = (r[right_neighbor_index] + g[right_neighbor_index] + b[right_neighbor_index]);
        float f_up = (r[up_neighbor_index] + g[up_neighbor_index] + b[up_neighbor_index]);
        float f_down = (r[down_neighbor_index] + g[down_neighbor_index] + b[down_neighbor_index]);

        float dx = -0.5 * f_left + 0.5 * f_right;
        float dy = -0.5 * f_down + 0.5 * f_up;
        float pixel_gradient = sqrt((dx * dx) + (dy * dy));
        
        

        if (pixel_gradient > gradient)
        {
