...
__global__ void process(int width, int height, float* r, float* g, float* b)
{
    ...
    if (global_pixel_col < width &&  global_pixel_row < height)
    {
        ...
        // Neighbor 1
        int neighbor_index_left = (global_pixel_col - 1) + global_pixel_row * width;
        blurred_r += r[neighbor_index_left];
        blurred_g += g[neighbor_index_left];
        blurred_b += b[neighbor_index_left];
        // Neighbor 2
        int neighbor_index_right = (global_pixel_col + 1) + global_pixel_row * width;
        blurred_r += r[neighbor_index_right];
        blurred_g += g[neighbor_index_right];
        blurred_b += b[neighbor_index_right];

        //Neighbor 3
        int neighbor_index_down = global_pixel_col + (global_pixel_row + 1) * width;
        blurred_r += r[neighbor_index_down];
        blurred_g += g[neighbor_index_down];
        blurred_b += b[neighbor_index_down];

        //Neighboy 4
        int neighbor_index_up = global_pixel_col + (global_pixel_row - 1) * width;
        blurred_r += r[neighbor_index_up];
        blurred_g += g[neighbor_index_up];
        blurred_b += b[neighbor_index_up];

        

        blurred_r = blurred_r/4;
        blurred_g = blurred_g/4;
        blurred_b = blurred_b/4;
...
int main(int argc, char *argv[])
{
    ...
    read("HW5pic.ppm", width, height, r,g,b);
